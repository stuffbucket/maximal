 ...
 */
// Shell-local copy of the live-feed wire contract (formerly the root repo's
// src/lib/ws/feed-types.ts). The engine now lives in @stuffbucket/maximal-core;
// these UI/wire types are shell-owned. settings-types comes from the core
// package (the same specifier the shell already uses across its features).
import type {

...
- `src/lib/config.ts` — `AppConfig` shape, disk read/write from `~/.local/share/maximal/config.json` (Linux/macOS) or `%APPDATA%\maximal\config.json` (Windows). Also respects `COPILOT_API_HOME` env var.
...
- `src/lib/secrets.ts` — file-based provider keys at `~/.local/share/maximal/secrets/<name>` (Linux/macOS) or `%APPDATA%\maximal\secrets\<name>` (Windows); mode 0600 enforced on POSIX, NTFS ACLs relied on for Windows. Env wins; file fills in unset values.
...
- **Daily log** at `~/.local/share/maximal/logs/messages-handler-<date>.log` (Linux/macOS) or `%APPDATA%\maximal\logs\messages-handler-<date>.log` (Windows) — request payloads, translated SSE events, web-tools agent traces. 7-day retention.

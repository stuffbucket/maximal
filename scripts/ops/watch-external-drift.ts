...
export function renderReport(results: ReadonlyArray<WatchResult>): string {
  ...
  const lines: Array<string> = [
    ...
    "",
    "The daily drift-watch workflow found upstream changes to sources `maximal` mirrors or impersonates. Each item below is scoped so a reconciliation PR can be derived from it directly.",
    "",
  ]
  ...
  lines.push(
    "---",
    "_Generated by the `watch-external-drift` workflow (`scripts/ops/watch-external-drift.ts`). Reused while drift persists; auto-closes on the next clean run. A maintainer — or `repoman`'s issue triage, where wired — derives the reconciliation PR from this issue._",
  )
  return lines.join("\n")
}

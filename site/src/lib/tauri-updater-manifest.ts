...
export function resolveUpdaterAssets(
  assets: UpdaterAsset[],
): { platform: TauriPlatform; bundle: UpdaterAsset; signatureAsset: UpdaterAsset } | null {
  ...
  const bundle = assets.find(
    (a) => /darwin-arm64\.app\.tar\.gz$/i.test(a.name),
  );
  if (!bundle) return null;
  const signatureAsset = assets.find(
    (a) => /darwin-arm64\.app\.tar\.gz\.sig$/i.test(a.name),
  );
